...
async def process_user_input(prompt: str, chat_id: int, session_id : str, previous_messages : List[str], current_title : str):
    try:
        ...
        if AIState.get_instance().get_chat_id() != chat_id:
            ...
            AIState.get_instance().set_user_input(prompt)
        ...
        final_state = await graph.ainvoke(initial_state, config={"configurable":graph_context}) #invoke the chatbot here
